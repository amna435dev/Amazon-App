...
function FootwearList() {
    ...
    // Fetch only footwear category
    useEffect(() => {
        dispatch(getAllProducts({ category: "Footwear", limit: 12 }));
    }, [dispatch]);
    ...
    return (
        <section className="max-w-7xl mx-auto px-4 py-8 grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {products.length === 0 ? (
                <p className="col-span-full text-center text-gray-500">
                    No products found in Footwear category.
                </p>
            ) : (
                products.map((p) => (
                    <motion.div
